...
local function getDeviceShort()
    ...
    if platform == Enum.Platform.IOS then
        ...
        if vp.X >= 1024 or vp.Y >= 1024 then return "iPad 📱"
        else return "iPhone 📱" end
    elseif platform == Enum.Platform.Android then return "Android 📱"
    elseif platform == Enum.Platform.Windows then return "Windows 💻"
    elseif platform == Enum.Platform.OSX then return "Mac 🖥️"
...
subLabel.BackgroundTransparency=1; subLabel.Text="FPS Monitor Ultimate v5.0"
...
infoLabel.Size=UDim2.new(1,0,0,24); infoLabel.Position=UDim2.new(0,0,1,-28)
infoLabel.BackgroundTransparency=1; infoLabel.Text="UG Hub v5.0 | by regretevator638"
infoLabel.Font=Enum.Font.Gotham; infoLabel.TextSize=10
infoLabel.TextColor3=Color3.fromRGB(80,80,80)
infoLabel.TextXAlignment=Enum.TextXAlignment.Center
infoLabel.ZIndex=51; infoLabel.Parent=loginFrame
...
-- ปุ่มล็อค เล็กๆ ติดอยู่กับ main frame
...
shotBtn.MouseButton1Click:Connect(function()
    settings.screenshotMode=not settings.screenshotMode
    mainFrame.Visible=not settings.screenshotMode
    shotBtn.BackgroundColor3=settings.screenshotMode and Color3.fromRGB(200,150,0) or Color3.fromRGB(60,60,60)
    task.delay(3,function()
        if settings.screenshotMode then
            settings.screenshotMode=false
            mainFrame.Visible=true
            shotBtn.BackgroundColor3=Color3.fromRGB(60,60,60)
        end
    end)
end)
...
graphFrame.Size=UDim2.new(1,0,0,22)
graphFrame.Position=UDim2.new(0,0,1,-44)
...
graphFrame.Active=true; graphFrame.Parent=mainFrame
...
local function updateGraph(fps)
    ...
    table.insert(fpsLog,{t=os.time(),v=fps})
...
miniFrame.Size=UDim2.new(0,72,0,22)
...
local tabNames={"🎨","⚙️","🎭","💾","👤","📊"}
local tabLabels={"Color","System","Theme","Preset","Info","Log"}
local tabPages={}
local tabBtns={}
...
for i,icon in ipairs(tabNames) do
    ...
    tl.Text=tabLabels[i]; tl.TextXAlignment=Enum.TextXAlignment.Center
...
local function createSlider(parent,labelText,yPos,minVal,maxVal,currentVal,step,zIdx,callback)
    local lbl=makeLabel(parent,UDim2.new(0,10,0,yPos),UDim2.new(1,-60,0,16),11,false,zIdx)
    lbl.Text=labelText
    local valLbl=makeLabel(parent,UDim2.new(1,-48,0,yPos),UDim2.new(0,40,0,16),11,true,zIdx)
    valLbl.Text=tostring(currentVal); valLbl.TextColor3=Color3.fromRGB(100,180,255)
...
local miniModeBtn=makeBtn(sysPage,UDim2.new(0,10,0,180),UDim2.new(1,-20,0,26),"📌 Mini Mode (F4)",Color3.fromRGB(100,60,160),22)
miniModeBtn.MouseButton1Click:Connect(function() setMiniMode(not miniMode) end)
local rejoinSysBtn=makeBtn(sysPage,UDim2.new(0,10,0,212),UDim2.new(1,-20,0,26),"🔄 Rejoin Game",Color3.fromRGB(40,100,200),22)
...
local function loadPreset(slot)
    ...
    mainFrame.BackgroundColor3=t.bg; mainGlow.Color=t.glow
...
for _,info in ipairs({{slot="A",label="Slot A",color=Color3.fromRGB(60,120,200),y=10},{slot="B",label="Slot B",color=Color3.fromRGB(150,60,200),y=58}}) do
    local lbl=makeLabel(presetPage,UDim2.new(0,10,0,info.y),UDim2.new(1,0,0,16),12,true,22); lbl.Text="💾 "..info.label
    local saveB=makeBtn(presetPage,UDim2.new(0,10,0,info.y+18),UDim2.new(0.46,0,0,24),"Save",info.color,22)
    local loadB=makeBtn(presetPage,UDim2.new(0.5,4,0,info.y+18),UDim2.new(0.46,-4,0,24),"Load",Color3.fromRGB(60,60,60),22)
    local s=info.slot
...
local vp=workspace.CurrentCamera.ViewportSize
local isVip=pcall(function() return game:GetService("MarketplaceService"):UserOwnsGamePassAsync(player.UserId,0) end)
local infoData={
    {"👤 User", player.Name},
...
for i,info in ipairs(infoData) do
    ...
    local kl=makeLabel(row,UDim2.new(0,6,0,0),UDim2.new(0.55,0,1,0),10,true,23); kl.Text=info[1]; kl.TextColor3=Color3.fromRGB(180,180,180)
    local vl=makeLabel(row,UDim2.new(0.55,0,0,0),UDim2.new(0.45,-6,1,0),10,false,23); vl.Text=info[2]; vl.TextXAlignment=Enum.TextXAlignment.Right; vl.TextColor3=Color3.fromRGB(100,200,255)
end
...
local logLayout=Instance.new("UIListLayout"); logLayout.SortOrder=Enum.SortOrder.LayoutOrder; logLayout.Parent=logScroll
...
local function updateLogDisplay()
    ...
    for i=#fpsLog,math.max(1,#fpsLog-29),-1 do
        ...
        lbl.BackgroundTransparency=count%2==0 and 0.8 or 1
        ...
        lbl.ZIndex=23; lbl.LayoutOrder=count; lbl.Parent=logScroll
        count+=1
    end
...
settingsBtn.MouseButton1Click:Connect(function()
    if panel.Visible and tabPages[6].Visible then updateLogDisplay() end
