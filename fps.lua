-- FPS Counter Ultimate Edition v3.1 (Mobile Friendly + Tabbed Settings)
...
addDrag(panel,nil)
addResizeHandle(panel,200,260,22,function(w,h) saveSettings() end)
...
local tabNames={"🎨","⚙️","🎭","💾"}
local tabLabels={"Color","System","Theme","Preset"}
local tabPages={}
local tabBtns={}

for i,icon in ipairs(tabNames) do
    local tb=makeBtn(tabBar,UDim2.new((i-1)/4,2,0,2),UDim2.new(1/4,-4,1,-4),icon,Color3.fromRGB(40,40,40),22)
    tb.TextSize=16
    tb.LayoutOrder=i
    tabBtns[i]=tb
    ...
    -- Tab label kecil
    local tl=makeLabel(tb,UDim2.new(0,0,0.5,2),UDim2.new(1,0,0,12),8,false,23)
    tl.Text=tabLabels[i]
...
for i,name in ipairs(themeNames) do
    ...
    tb.MouseButton1Click:Connect(function()
        ...
        for n,b in pairs(themeBtns) do
            b.BackgroundColor3=n==name and Color3.fromRGB(60,180,100) or Color3.fromRGB(50,50,50)
        end
        saveSettings()
    end)
end
...
local function loadPreset(slot)
    ...
    settings.bgTransparency=p.bgTransparency
    settings.autoColor=p.autoColor
    settings.theme=p.theme or "Dark"
    mainFrame.BackgroundTransparency=settings.bgTransparency
...
local slotInfo={
    {slot="A",label="Slot A",color=Color3.fromRGB(60,120,200),y=10},
    {slot="B",label="Slot B",color=Color3.fromRGB(150,60,200),y=58},
}
for _,info in ipairs(slotInfo) do
    ...
    saveB.MouseButton1Click:Connect(function()
        savePreset(s)
        saveB.Text="Saved ✓"
        task.delay(1.5,function() saveB.Text="Save" end)
    end)
    loadB.MouseButton1Click:Connect(function()
        loadPreset(s)
        loadB.Text="Loaded ✓"
        task.delay(1.5,function() loadB.Text="Load" end)
    end)
end
